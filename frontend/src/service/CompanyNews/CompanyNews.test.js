...
import { getCompanyNews } from "./CompanyNews";

jest.mock("axios")

describe("Company News Service", () => {
    it("should get company news", async () => {
        const companyNews = [
            {
        ...
        axios.get.mockImplementationOnce(() => Promise.resolve(companyNews));
        await expect(getCompanyNews(companyName)).resolves.toEqual(companyNews);
        expect(axios.get).toHaveBeenCalledWith(NEWS_API_URL + companyName);
    });
});